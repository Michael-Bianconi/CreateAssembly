class Assembler {
    ...
    private static toOpcode(mnemonic: string, operands: string[]): number | null {
    ...
    private static parse(operands: string[], parsers: ((s: string) => (string | number | null))[]): (string | number)[] | null {
        if (operands.length === parsers.length) {
            let ops: (string | number)[] = [];
            for (let i = 0; i < operands.length; i++) {
                let result = parsers[i](operands[i]);
                if (result !== null) {
                    ops.push(result);
                } else {
                    return null;
    ...
    private static parseV(operand: string): string | number | null {
        let match = operand.match(/^V([0-9A-F])$/);
        return match !== null ? parseInt(match[1], 16) : null;
    }

    private static parseV0(operand: string): string | number | null {
        return Assembler.parseLiteral('V0', operand);
    }

    private static parseI(operand: string): string | number | null {
        return Assembler.parseLiteral('I', operand);
    }

    private static parseB(operand: string): string | number | null {
        return Assembler.parseLiteral('B', operand);
    }

    private static parseF(operand: string): string | number | null {
        return Assembler.parseLiteral('F', operand);
    }

    private static parseDT(operand: string): string | number | null {
        return Assembler.parseLiteral('DT', operand);
    }

    private static parseST(operand: string): string | number | null {
        return Assembler.parseLiteral('ST', operand);
    }

    private static parseK(operand: string): string | number | null {
        return Assembler.parseLiteral('K', operand);
    }

    private static parseAI(operand: string): string | number | null {
        return Assembler.parseLiteral('[I]', operand);
    }

    private static parseNibble(operand: string): string | number | null {
        let value = parseInt(operand);
        return !isNaN(value) && 0 <= value && value <= 0xF ? value : null;
    }

    private static parseByte(operand: string): string | number | null {
        let value = parseInt(operand);
        return !isNaN(value) && 0 <= value && value <= 0xFF ? value : null;
    }

    private static parseAddress(operand: string): string | number | null {
        let value = parseInt(operand);
        return !isNaN(value) && 0 <= value && value <= 0xFFF ? value : null;
    }

    private static parseLiteral(literal: string, operand: string): string | number | null {
